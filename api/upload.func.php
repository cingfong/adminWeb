 ...
 */
function uploadFile($fileInfo, $allowExt = array('jpeg', 'jpg', 'gif', 'png'), $maxSize = 2097152, $flag = true, $uploadPath = './static/img/news') {
    // 存放錯誤訊息
    $res = array();
    //取得上傳時間點
    $name_time = time();
    ...
    if ($fileInfo['error'] > 0) {
        ...
        switch ($fileInfo['error']) {
            ...
            //     // 但若無上傳則在原本上傳的ＰＨＰ文間判斷
//  獲取 sql 內 live 的值，更合理情況應該是在外面判斷較好
                // $res['mes'] = $old_news_live_array . ' 沒有檔案被上傳（沒有選擇上傳檔案就送出表單）';
                $res['dest'] = '';
                break;
            case 6:
